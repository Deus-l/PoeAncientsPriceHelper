...
internal sealed class AppConfig
{
    ...
    // OCR language passed to Tesseract ("eng" or "rus"). Must match a .traineddata file that
    // actually exists in the published tessdata/ folder — "eng" ships via the Tesseract.Data.English
    // NuGet package, "rus" ships as a checked-in file (see PoeAncientsPriceHelper.csproj).
    public string OcrLanguage { get; set; } = "eng";
    // Languages offered in the settings dropdown. [JsonIgnore] for the same reason as
    // AvailableLeagues above — this is a code-defined constant, not user data.
    [JsonIgnore]
    public List<string> AvailableOcrLanguages { get; set; } = ["eng", "rus"];
