...
internal sealed class PriceOverlayForm : Form
{
    ...
    private void PaintScene(Graphics g)
    {
        ...
        // Box geometry matches the primary slice fed to OCR (left glyph column cut, right border trimmed).
        int ocrLeft = _regionRect.Left + OcrScanner.CalculateLeftCut(_regionRect.Width);
        int ocrRight = _regionRect.Right - OcrScanner.CalculateRightCut(_regionRect.Width);
