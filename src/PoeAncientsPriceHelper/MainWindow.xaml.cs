...
public partial class MainWindow : MetroWindow
{
    ...
    private void PopulateFields()
    {
        ...
        OcrLanguageBox.ItemsSource = _config.AvailableOcrLanguages;
        OcrLanguageBox.SelectedItem = _config.AvailableOcrLanguages.Contains(_config.OcrLanguage)
            ? _config.OcrLanguage
            : _config.AvailableOcrLanguages.FirstOrDefault();
    ...
    // Unlike league, changing the OCR language doesn't need a price re-fetch — poe.ninja's price
    // snapshot is always English regardless of which Tesseract language reads the overlay. The new
    // value just needs to be persisted; ScanEngine picks it up next time scanning starts (it reads
    // _config.OcrLanguage when it constructs the OcrScanner, not at app startup).
    private void OcrLanguageBox_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e)
    {
        if (_loading || OcrLanguageBox.SelectedItem is not string lang || lang == _config.OcrLanguage) return;
        _config.OcrLanguage = lang;
        ConfigStore.Save(_config);
    }
