...
internal sealed class ScanEngine : IDisposable
{
    ...
    private async Task RunLoopAsync(CancellationToken ct)
    {
        ...
        using var scanner = new OcrScanner(tessdataDir, _config.OcrLanguage, Log, App.DebugMode);
        ...
        int zeroRowStreak = 0;        // consecutive OCR-zero cycles while confirmed-open — detects walk-away close
        const int ZeroRowsToClose = 5; // ~500ms of empty OCR → treat as panel closed (walk-away case)
        int cycleCount = 0;
                    ...
                    if (isOpen)
                    {
                        ...
                        if ((now - lastOcrAt).TotalMilliseconds >= MinOcrIntervalMs)
                        {
                            ...
                            if (ocrRows.Count == 0)
                            {
                                ...
                                // But if this keeps happening while we thought the panel was confirmed
                                // open, it means the user walked away and the brightness didn't drop
                                // (game world reads bright). After ZeroRowsToClose consecutive empty
                                // scans, collapse the confirmed state so the overlay hides quickly.
                                Log("OCR returned 0 rows");
                                if (confirmedOpen)
                                {
                                    zeroRowStreak++;
                                    if (zeroRowStreak >= ZeroRowsToClose)
                                    {
                                        Log($"zero-row streak ({zeroRowStreak}) → panel closed (walk-away)");
                                        confirmedOpen = false;
                                        slots.Clear();
                                        lastRows = [];
                                        zeroRowStreak = 0;
    ...
    private IReadOnlyList<PriceRow> BuildPriceRows(IReadOnlyList<OcrRow> ocrRows)
    {
        ...
        {
            // In Russian OCR mode the OCR text is Cyrillic — translate it to the EN canonical name
            // via RuItemNames BEFORE any of the logic below runs. Every check from here down (gem
            // detection, easter eggs, exact/prefix/fuzzy price match) is keyed on English words, so
            // without this translation step nothing would ever match a Russian-language row.
            // Untranslated text passes through unchanged and falls through to '?', same as an
            // unrecognised English row today — it never blocks the eng-mode path (no-op when
            // OcrLanguage != "rus").
            ...
            if (TryResolveGemKey(name, out var gemKey))
            {
                if (gemKey is not null && snapshot.TryGetValue(gemKey, out var gemEntry))
                    ...
                    rows.Add(new PriceRow(stableY, row.RawText, 0m, 0m, false, row.Multiplier, name));
                continue;
            }
                                  ...
                                  .MinBy(k => k.Length) is { } prefixKey)
            {
                ...
                matchedKey = prefixKey;
                     ...
                     BestFuzzy(snapshot, name) is { } fuzzy)
            {
    ...
    // RU OCR mode only (no-op / passthrough otherwise): resolves a normalized Cyrillic OCR string
    // to its EN canonical normalized name via RuItemNames (exact match, then fuzzy against the RU
    // dictionary keys so a couple of misread Cyrillic letters don't break the match — mirrors
    // BestFuzzy below, just matched against RU names instead of price-snapshot keys). Falls back to
    // the original text when no translation is found, which simply won't match anything further
    // down and renders as '?' — the same degrade-gracefully behaviour as an unknown English name.
    private string ResolveEffectiveName(string normalizedName) =>
        _config.OcrLanguage == "rus" ? RuItemNames.Resolve(normalizedName) ?? normalizedName : normalizedName;
